            ...
            <div>
                ...
                <ul className="masonry">
                    {Object.keys(boardPins).map((id) => {
                        if(pins[id]){
                            return (
                                <li key={id} className="masonry-image"><svg onClick={()=>this.props.removePinFromBoard({board_id: this.props.boardId, pin_id: id}).then(() => this.props.getBoard(this.props.boardId))} className="unpin-icon" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                                    width="26" height="26"
                                    viewBox="0 0 172 172"
